...
@InteractionConstraintDeclaration(FirstPage.FirstPageConstraint.class)
...
public @interface FirstPage
{
   ...
   class FirstPageConstraint
         implements InteractionConstraint<FirstPage>
   {

      @Structure
      Module module;

      public boolean isValid( FirstPage firstPage, RoleMap roleMap )
      {
         Pages.Data pages = roleMap.get( Pages.Data.class );
         Page page = roleMap.get( Page.class );
         if( firstPage.value() )
         {
            return pages.pages().toList().indexOf( page ) == 0;
         } else
         {
            return pages.pages().toList().indexOf( page ) != 0;
         }

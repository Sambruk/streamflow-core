    ...
    extends WizardBranchController
{
    ...
    protected WizardPanelProvider getPanelProviderForStep(String name, Map map)
    {
        if (map.get(FormsWizardPage.SELECTED_FORM) != null)
        {
            ...
            String[] pageIds = new String[] {value.entity().get().identity()};
            String[] pageNames = new String[] {title};

            FormSubmitWizardPage submitView = obf.newObjectBuilder(FormSubmitWizardPage.class)
                    .use(model.getFormSubmitModel(value.entity().get().identity())).newInstance();

            return new FormSubmissionWizardPanelProvider(submitView, title, pageIds, pageNames);
        }
        return null;
    }
}